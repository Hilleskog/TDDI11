...
int main(int argc, char **argv)
{
	...
	while(true)
	{
		...
		isAcOn = (((inputRegister >> 4) & 0x00000001) == 0x00000001); //isAcOn = false; // Assignment
		// bits 5 to 9 indicate desired temperature
		unsigned int temperatureDesired_tmp = ((inputRegister >> 5) & 0x1F); // Assignment
		...
		unsigned int temperatureMeasured_fractional = ((inputRegister >> 10) & 0x7); // Assignment
		//temperatureMeasured_fractional = temperatureMeasured_fractional * 0.2; // Assignment
		// bits 13 to 17 are decimal part
		unsigned int temperatureMeasured_decimal = ((inputRegister >> 13) & 0x1F) ; // Assignment
		//temperatureMeasured_decimal = 0x00000000; // Assignment
		// According to equation 2:
		if (temperatureMeasured_decimal == 0x1F){ // Assignment
			...
			temperatureMeasured = temperatureMeasured_decimal * 1.0 + temperatureMeasured_fractional * 0.2; // Assignment
		} // Assignment
		...
		if(fanSpeedSelector == 0x0) // FULL SPEEEEEED
			fanSpeed = 0x3; // Assignment
		else if (fanSpeedSelector == 0x5) // Assignment // MEDIUM
			fanSpeed = 0x2; // Assignment
		else if (fanSpeedSelector == 0x3) // Assignment // LOW
			fanSpeed = 0x1; // Assignment
		else
			fanSpeed = 0x0; // Assignment // OFF
		...
		if (isAcOn)
		{
			if((temperatureMeasured - temperatureDesired) > Hysteresis_Cooler)
			{
				isCoolerOn = true;
				isHeaterOn = false;
			} else if ((temperatureMeasured - temperatureDesired) < Hysteresis_Heater) // Assignment
			{
				isCoolerOn = false;
				isHeaterOn = true;
			} else {
		...
		if (isHeaterOn && (temperatureMeasured >= temperatureDesired)) // Assignment
		{
			isCoolerOn = false;
			isHeaterOn = false;
		} // Assignment
